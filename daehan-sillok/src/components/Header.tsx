...
export default function Header() {
  ...
  const activeTab =
    location.pathname.startsWith("/history")
      ? "history"
      : location.pathname.startsWith("/folktales")
      ? "folktales"
      : "home";

  const tabs = [
    { id: "home", label: "홈", path: "/" },
    { id: "history", label: "역사", path: "/history" },
    { id: "folktales", label: "설화·신화", path: "/folktales" },
  ] as const;
  ...
  const getHeaderStyles = () => {
    if (activeTab === "home") {
      return {
        headerBg:
          "bg-[#181513]/95 border-b border-[#D4AF37]/15 text-[#E8E3D9]",
        logoAccentbg:
    ...
    if (activeTab === "folktales") {
      return {
        ...
        logoAccentbg:
          "bg-[#008B8B]/20 border border-[#00FA9A]/30 text-[#00FA9A]",
    ...
    return {
      headerBg:
        "bg-[#1E0402]/95 border-b border-[#D4AF37]/25 text-[#E8E3D9]",
      logoAccentbg:
        "bg-[#D4AF37]/10 border border-[#D4AF37]/35 text-[#D4AF37]",
  ...
  return (
    <header
      className={`sticky top-0 z-50 w-full backdrop-blur-md px-4 sm:px-8 py-3.5 flex items-center justify-between shadow-xs transition-colors duration-500 ${styles.headerBg}`}
    >
      <div
        onClick={() => navigate("/")}
      ...
      >
        <div
          className={`relative w-10 h-10 flex items-center justify-center font-serif font-black text-base border transition-all ${styles.logoAccentbg} rounded-none`}
        >
          <span>실</span>
        </div>
        ...
        <div>
          <h1
            className={`text-xl sm:text-2xl font-serif font-black tracking-widest flex items-center gap-2 ${styles.textPrimary}`}
          >
            대한실록
          </h1>
